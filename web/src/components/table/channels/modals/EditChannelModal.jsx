...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    case 50:
      return '按照如下格式输入: AccessKey|SecretKey，如果上游是兼容 OpenAI 的中转服务，则直接输入 ApiKey';
    case 51:
                      ...
                      {inputs.type === 8 && (
                        <>
                          <Banner
                            type='warning'
                            description={t(
                              '如果你对接的是上游 One API 或其他兼容 OpenAI 的转发项目，请使用 OpenAI 类型，不要使用此类型，除非你明确知道自己在做什么。',
                            )}
                            className='!rounded-lg'
                          />
                          <div>
                        ...
                        (inputs.type !== 45 || doubaoApiEditUnlocked) && (
                          <div>
                            <Form.Input
                              ...
                              disabled={isIonetLocked}
                              extraText={t(
                                '对于官方渠道，系统已经内置地址，除非是第三方代理站点或者 Azure 的特殊接入地址，否则不需要填写',
                              )}
                            />
                          </div>
                        )}
