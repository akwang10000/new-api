...
export default function SettingsPaymentGateway(props) {
  ...
  return (
    <Spin spinning={loading}>
      ...
      >
        <Form.Section text={t('MotionPay 支付设置')}>
          <Text>
            {t(
              '当前通过 MotionPay（易支付兼容接口）发起微信/支付宝支付，支付地址请填写网关根地址，例如 https://motionpay.net',
            )}
          </Text>
          <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 24, xl: 24, xxl: 24 }}>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.Input
                field='PayAddress'
                label={t('MotionPay 地址')}
                placeholder={t('例如：https://motionpay.net')}
              />
            </Col>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.Input
                field='EpayId'
                label={t('MotionPay 商户ID')}
          ...
          >
            ...
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.InputNumber
                ...
                precision={2}
                label={t('充值价格（x元/美元）')}
                placeholder={t('例如：7，表示 1 美元按 7 元计价')}
              />
            </Col>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.InputNumber
          ...
          <Form.TextArea
            ...
            label={t('充值分组倍率')}
            placeholder={t('填写 JSON 文本，键为分组名称，值为倍率')}
            autosize
          />
          <Form.TextArea
            field='PayMethods'
            label={t('充值方式设置')}
            placeholder={t('填写 JSON 文本')}
            autosize
          />
          ...
          >
            <Col span={24}>
              <Form.TextArea
                ...
                label={t('自定义充值金额选项')}
                placeholder={t(
                  '填写 JSON 数组，例如：[10, 20, 50, 100, 200, 500]',
                )}
                autosize
                extraText={t(
                  '设置用户可选择的充值金额选项，例如：[10, 20, 50, 100, 200, 500]',
                )}
              />
            </Col>
          </Row>
          ...
          >
            <Col span={24}>
              <Form.TextArea
                ...
                label={t('充值金额折扣配置')}
                placeholder={t(
                  '填写 JSON 对象，例如：{"100": 0.95, "200": 0.9, "500": 0.85}',
                )}
                autosize
                extraText={t(
                  '设置不同充值金额对应的折扣率，键为充值金额，值为折扣率',
                )}
              />
            </Col>
          </Row>
